...
static NSURL *PSCInstallFileURLToDocumentFolderAndOverride(NSURL *documentURL, BOOL override, BOOL copy) {
    ...
    if (!exists || override) {
        ...
        if (copy) {
            ...
            if (![NSFileManager.defaultManager moveItemAtURL:documentURL toURL:newURL error:&error]) {
...
BOOL PSCIsFileLocatedInSamplesFolder(NSURL *documentURL) {
    NSString *samplesURL = [NSBundle.mainBundle.resourceURL URLByAppendingPathComponent:@"Samples"].path;
    // We need to resolve symlinks using `URLByResolvingSymlinksInPath` to strip the private designator in the start of the URL.
    return [documentURL.URLByResolvingSymlinksInPath.path hasPrefix:samplesURL.stringByResolvingSymlinksInPath];
}
