...
private class ElearningPDFViewController: PDFViewController {
    ...
    init(with example: IndustryExample) {
        studentDocument = AssetLoader.writableDocument(withName: .student, overrideIfExists: true)
        teacherDocument = AssetLoader.writableDocument(withName: .teacher, overrideIfExists: true)

        super.init(document: studentDocument, configuration: nil)

        // Asynchronously pre-render and cache the documents.
        // See https://pspdfkit.com/guides/ios/current/getting-started/rendering-pdf-pages/#the-cache for more details.
