...
- (nullable UIImage *)coverImageForSize:(CGSize)size {
    ...
    coverImage = [PSPDFKitGlobal.sharedInstance.cache imageForRequest:request imageSizeMatching:PSPDFCacheImageSizeMatchingDefault error:NULL];
...
- (PSPDFViewState *)lastViewState {
    ...
    if (self.isValid) {
        ...
        if (viewStateData) {
            NSError *error;
            viewState = [NSKeyedUnarchiver unarchivedObjectOfClass:PSPDFViewState.class fromData:viewStateData error:&error];
            if (error) {
                PSCLog(@"Failed to load saved viewState: %@", error.localizedDescription);
...
- (void)setLastViewState:(PSPDFViewState *)lastViewState {
    if (self.isValid) {
        if (lastViewState) {
            NSData *viewStateData = [NSKeyedArchiver archivedDataWithRootObject:lastViewState requiringSecureCoding:YES error:NULL];
