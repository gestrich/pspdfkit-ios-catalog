...
- (void)viewDidLoad {
    ...
    UISearchBar *searchBar = [[UISearchBar alloc] initWithFrame:CGRectIntegral(CGRectMake((self.collectionView.bounds.size.width - searchBarWidth) / 2, -44.0, searchBarWidth, 44.0))];
    searchBar.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin;
    searchBar.tintColor = UIColor.psc_textColor;
    ...
    PSCGetViewInsideView(searchBar, @"UISearchBarBack").alpha = 0;
...
- (void)setProgressIndicatorVisible:(BOOL)visible animated:(BOOL)animated {
    if (visible) {
        if (!self.activityView) {
            UIActivityIndicatorView *activityView = PSC_CATALYST_DEPRECATED_NOWARN([[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhiteLarge];)
...
// Calculates where the document view will be on screen.
- (CGRect)magazinePageCoordinatesWithPDFController:(PSCMagazineKioskPDFViewController *)pdfController {
    CGRect newFrame = self.view.frame;
...
- (BOOL)openMagazine:(PSCMagazine *)magazine animated:(BOOL)animated cellIndex:(NSUInteger)cellIndex {
    ...
    PSCMagazineKioskPDFViewController *pdfController = [[PSCMagazineKioskPDFViewController alloc] initWithDocument:magazine configuration:[PSPDFConfiguration configurationWithBuilder:^(PSPDFConfigurationBuilder *builder) {
    ...
    PSPDFRenderTask *coverTask = [[PSPDFRenderTask alloc] initWithRequest:request error:NULL];
    ...
    if (animated && pdfController.pageIndex != 0) {
        ...
        targetPageTask = [[PSPDFRenderTask alloc] initWithRequest:request error:NULL];
        targetPageTask.priority = PSPDFRenderQueuePriorityUserInteractive;
        [tasks addObject:targetPageTask];
    }

    [PSPDFKitGlobal.sharedInstance.renderManager.renderQueue scheduleTasks:tasks];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)layout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    const UICollectionViewFlowLayout *flowLayout = (UICollectionViewFlowLayout *)layout;
    const CGSize containerSize = UIEdgeInsetsInsetRect(collectionView.bounds, flowLayout.sectionInset).size;

    return [PSPDFThumbnailViewController automaticThumbnailSizeForPageWithSize:pageSize referencePageSize:ASeriesSize containerSize:containerSize interitemSpacing:flowLayout.minimumInteritemSpacing];
}
...
- (id<UIViewControllerAnimatedTransitioning>)navigationController:(UINavigationController *)navigationController animationControllerForOperation:(UINavigationControllerOperation)operation fromViewController:(UIViewController *)fromVC toViewController:(UIViewController *)toVC {
    BOOL pushingMagazine = operation == UINavigationControllerOperationPush && fromVC == self && [toVC isKindOfClass:[PSCMagazineKioskPDFViewController class]];
    BOOL popMagazine = operation == UINavigationControllerOperationPop && toVC == self && [fromVC isKindOfClass:[PSCMagazineKioskPDFViewController class]];
...
- (void)animateZoomInTransition:(id<UIViewControllerContextTransitioning>)transitionContext {
    ...
    PSCMagazineKioskPDFViewController *pdfController = (PSCMagazineKioskPDFViewController *)toViewController;
...
- (void)animateZoomOutTransition:(id<UIViewControllerContextTransitioning>)transitionContext {
    UIView *containerView = transitionContext.containerView;
    UIViewController *toViewController = [transitionContext viewControllerForKey:UITransitionContextToViewControllerKey];
    PSCMagazineKioskPDFViewController *fromViewController = [transitionContext viewControllerForKey:UITransitionContextFromViewControllerKey];
    NSParameterAssert([fromViewController isKindOfClass:PSCMagazineKioskPDFViewController.class]);
    ...
    if (lastOpenedMagazine) {
        ...
        updatedImage = [PSPDFKitGlobal.sharedInstance.cache imageForRequest:request imageSizeMatching:PSPDFCacheImageSizeMatchingAllowSmaller error:NULL];
    }
