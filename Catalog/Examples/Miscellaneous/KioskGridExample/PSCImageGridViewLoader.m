...
- (id<PSPDFPageCellImageRequestToken>)requestImageForPageAtIndex:(PSPDFPageIndex)pageIndex availableSize:(CGSize)size completionHandler:(void (^)(UIImage *_Nullable, NSError *_Nullable))completionHandler {
    ...
    if (magazine.isAvailable) {
        ...
        UIImage *memoryImage = [PSPDFKitGlobal.sharedInstance.cache imageForRequest:request imageSizeMatching:PSPDFCacheImageSizeMatchingDefault error:NULL];
        if (memoryImage) {
        ...
        PSPDFRenderTask *task = [[PSPDFRenderTask alloc] initWithRequest:request error:NULL];
