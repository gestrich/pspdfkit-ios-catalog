...
- (void)layoutSubviews {
    ...
    self.deleteButton.frame = CGRectMake(self.imageView.frame.origin.x - 10.0, self.imageView.frame.origin.y - 10.0, self.deleteButton.frame.size.width, self.deleteButton.frame.size.height);
...
- (CGRect)imageRectForContentRect:(CGRect)contentRect {
    ...
    if (image) {
        ...
        imageRect.origin.x = contentRect.origin.x + (CGRectGetWidth(contentRect) / 2.0 - CGRectGetWidth(imageRect) / 2.0);
        imageRect.origin.y = contentRect.origin.y + (CGRectGetHeight(contentRect) / 2.0 - CGRectGetHeight(imageRect) / 2.0);
...
- (void)setMagazineCount:(NSUInteger)magazineCount {
    ...
    if (!_magazineCounter && magazineCount > 1) { // lazy creation
        ...
        _magazineCounter.font = [UIFont boldSystemFontOfSize:20.0];
