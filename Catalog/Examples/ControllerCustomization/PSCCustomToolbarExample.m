...
- (void)commonInitWithDocument:(PSPDFDocument *)document configuration:(PSPDFConfiguration *)configuration {
    ...
    UISegmentedControl *customViewModeSegment = [[UISegmentedControl alloc] initWithItems:@[@"Page", @"Thumbnails"]];
...
/*
 Implemented just for your curiosity.
 You can use that to add custom loading views to the PSPDFScrollView.
 */
...
- (void)pdfViewController:(PSPDFViewController *)pdfController willScheduleRenderTaskForPageView:(PSPDFPageView *)pageView {
    ...
    if (!indicator) {
        indicator = PSC_CATALYST_DEPRECATED_NOWARN([[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
        [indicator sizeToFit];)
        [indicator startAnimating];
        indicator.tag = PSPDFLoadingViewTag;
        indicator.frame = CGRectMake(__tg_floor((pageView.frame.size.width - indicator.frame.size.width) / 2.0), __tg_floor((pageView.frame.size.height - indicator.frame.size.height) / 2.0), indicator.frame.size.width, indicator.frame.size.height);
