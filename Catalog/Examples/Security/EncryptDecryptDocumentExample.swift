...
class EncryptDecryptDocumentExample: Example {
    ...
    override func invoke(with delegate: ExampleRunnerDelegate) -> UIViewController {
        let originalURL = AssetLoader.assetURL(withName: .JKHF)
        let cryptor = Cryptor()
        let key = cryptor.key(fromPassphrase: "passphrase", salt: "salt")

        // Encrypt the original PDF
        let encryptedURL = TempFileURLWithPathExtension(prefix: "encrypted", pathExtension: "pdf")
        do {
        ...
        let decryptedURL = TempFileURLWithPathExtension(prefix: "PSPDFCryptorTests", pathExtension: "pdf")
        do {
