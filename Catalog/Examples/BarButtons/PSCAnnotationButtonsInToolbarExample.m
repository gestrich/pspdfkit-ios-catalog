...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *inkBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[PSPDFKitGlobal imageNamed:@"ink"] style:UIBarButtonItemStylePlain target:self action:@selector(inkBarButtonPressed:)];
    UIBarButtonItem *eraserBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[PSPDFKitGlobal imageNamed:@"eraser"] style:UIBarButtonItemStylePlain target:self action:@selector(eraserBarButtonPressed:)];
...
- (void)observeValueForKeyPath:(nullable NSString *)keyPath ofObject:(nullable id)object change:(nullable NSDictionary<NSString *, id> *)change context:(nullable void *)context {
    if (context == PSCAnnotationStateManagerChangedStateContext) {
        if ([keyPath isEqualToString:NSStringFromSelector(@selector(state))]) {
            ...
            if (inkEnabled) {
                inkBarButtonItem.image = [PSPDFKitGlobal imageNamed:@"x"];
            } else {
                inkBarButtonItem.image = [PSPDFKitGlobal imageNamed:@"ink"];
            }

            if (eraserEnabled) {
                eraserBarButtonItem.image = [PSPDFKitGlobal imageNamed:@"x"];
            } else {
                eraserBarButtonItem.image = [PSPDFKitGlobal imageNamed:@"eraser"];
            }
        }
    } else {
