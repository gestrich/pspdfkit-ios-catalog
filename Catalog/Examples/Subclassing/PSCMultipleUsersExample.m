...
- (instancetype)init {
    if ((self = [super init])) {
        self.title = @"Store annotations in external files for multiple users";
        self.contentDescription = @"Shows how to store annotations in external files for multiple users";
...
// In a real application, this could be a lot more elaborate - e.g. showing all available users in a nice table view with edit/delete, etc.
- (void)switchUser {
