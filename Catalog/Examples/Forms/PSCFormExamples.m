...
- (nullable UIViewController *)invokeWithDelegate:(id<PSCExampleRunnerDelegate>)delegate {
    ...
    /*
     The document created on digitally singing the document here will allow editing of annotations.
     Check out our Digital Signatures guide for more information.
     https://pspdfkit.com/guides/ios/current/features/digital-signatures/#editing-a-digitally-signed-document
     */
        ...
        for (PSPDFFormElement *formElement in annotations) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                if ([formElement isKindOfClass:PSPDFTextFieldFormElement.class]) {
                    ...
                    NSString *fieldName = formElement.fieldName;

                    if (textFieldElement.inputFormat == PSPDFTextInputFormatDate) {
                        textFieldElement.contents = @"01/01/2001";
                    // Telephone_Home needs exactly 7 digits
                    } else if ([fieldName  isEqual: @"Telephone_Home"]) {
                        textFieldElement.contents = @"0123456";
                        // Social Security Number needs exactly 9 digits
                    } else if ([fieldName  isEqual: @"SSN"]) {
                        textFieldElement.contents = @"012345678";
                        // The other phone numbers need exactly 10 digits
                    } else if ([fieldName  isEqual: @"Telephone_Work"] || [fieldName  isEqual: @"Emergency_Phone"]) {
...
@interface PSCFormWithFormattingExample : PSCExample
@end
@implementation PSCFormWithFormattingExample
...
@interface PSCFormWithFormattingReadonlyExample : PSCExample
@end
@implementation PSCFormWithFormattingReadonlyExample
