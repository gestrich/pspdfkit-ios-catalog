...
class AsynchronousExternalDigitalSignatureExample: Example {
            ...
            DispatchQueue.main.async(execute: { () -> Void in
                ...
                alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { _ in
                    if let password = alert.textFields?.first?.text {
                        let certURL = AssetLoader.assetURL(withName: "JohnAppleseed.p12")
    ...
    override func invoke(with delegate: ExampleRunnerDelegate) -> UIViewController? {
        ...
        let unsignedDocument = AssetLoader.writableDocument(withName: "Form_example.pdf", overrideIfExists: true)
