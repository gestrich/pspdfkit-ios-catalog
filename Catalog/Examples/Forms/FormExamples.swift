...
class FormInteractiveDigitalSigningExample: Example {
    ...
    override func invoke(with delegate: ExampleRunnerDelegate) -> UIViewController {
        ...
        p12signer.signersName = "John Thomas Appleseed"
...
class FormDigitalSigningExample: Example {
    ...
    override func invoke(with delegate: ExampleRunnerDelegate) -> UIViewController {
        ...
        let unsignedDocument = AssetLoader.document(withName: "Form_example.pdf")
...
class FormDigitalSigningExampleCustomAppearanceExample: Example {
    ...
    // MARK: Automated digital signing process with custom appearance

    override init() {
        super.init()
        title = "Automated digital signing process with custom appearance"
        category = .forms
        priority = 16
    }
    ...
    override func invoke(with delegate: ExampleRunnerDelegate) -> UIViewController {
        ...
        let tempPDF = TempFileURLWithPathExtension(prefix: "appearance", pathExtension: "pdf")
...
class FormFillingExample: Example {
    ...
    override func invoke(with delegate: ExampleRunnerDelegate) -> UIViewController {
        let document = AssetLoader.document(withName: "Form_example.pdf")
...
private final class FormFillingPDFViewController: PDFViewController {
    ...
    private func saveCopy(_ sender: UIBarButtonItem) {
        ...
        let tempURL = TempFileURLWithPathExtension(prefix: "copy_\(document?.fileURL?.lastPathComponent ?? "Form_example")", pathExtension: "pdf")
...
class FormDigitallySignedModifiedExample: Example {
    ...
    override func invoke(with delegate: ExampleRunnerDelegate) -> UIViewController {
        let document = AssetLoader.document(withName: "Form_example_signed.pdf")
...
class FormWithFormattingExample: Example {
    ...
    override func invoke(with delegate: ExampleRunnerDelegate) -> UIViewController {
        let document = AssetLoader.document(withName: "Forms_formatted.pdf")
        return PDFViewController(document: document)
    }
}
