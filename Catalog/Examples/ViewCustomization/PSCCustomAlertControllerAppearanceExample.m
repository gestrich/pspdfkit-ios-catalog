...
@interface PSCCustomAlertControllerAppearanceExample : PSCExample <PSPDFViewControllerDelegate>
@end

@implementation PSCCustomAlertControllerAppearanceExample
...
- (BOOL)pdfViewController:(PSPDFViewController *)pdfController shouldShowController:(UIViewController *)controller options:(nullable NSDictionary<NSString *, id> *)options animated:(BOOL)animated {
    ...
    if ([controller isKindOfClass:UIAlertController.class]) {
        dispatch_async(dispatch_get_main_queue(), ^{
            controller.view.tintColor = UIColor.systemOrangeColor;
        });
    }
