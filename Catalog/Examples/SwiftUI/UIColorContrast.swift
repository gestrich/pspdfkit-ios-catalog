...
extension UIColor {
    ...
    class func isColorAboutEqual(_ left: UIColor?, to right: UIColor?, tolerance: CGFloat = 0.01) -> Bool {
        guard let leftColor = left?.inRGBColorSpace.cgColor,
              ...
              leftColor.colorSpace?.model == rightColor.colorSpace?.model else { return false }
        ...
        for i in 0..<componentCount {
            if CGFloat(abs(Float((leftComponents?[i] ?? 0.0) - (rightComponents?[i] ?? 0.0)))) > tolerance {
                return false
            }
        }
        return true
...
extension UIColor {
    ...
    var inRGBColorSpace: UIColor {
        guard cgColor.colorSpace?.model != .rgb else { return self }
        // convert UIDeviceWhiteColorSpace to UIDeviceRGBColorSpace.
        let components = cgColor.components
        let whiteComponent = components?[0] ?? 0.0
        return UIColor(red: whiteComponent, green: whiteComponent, blue: whiteComponent, alpha: components?[1] ?? 0.0)
    }
