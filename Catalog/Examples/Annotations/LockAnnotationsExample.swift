...
class LockAnnotationsExample: Example {
    ...
    override func invoke(with delegate: ExampleRunnerDelegate) -> UIViewController {
        ...
        let writableDocumentURL = CopyFileURLToDocumentFolder(documentURL, override: false)

        // Copy the document to the temp directory.
        let tempURL = TempFileURLWithPathExtension(prefix: "locked_\(writableDocumentURL.lastPathComponent)", pathExtension: "pdf")
        ...
        if allAnnotations.isEmpty {
            let ink = InkAnnotation.psc_sampleInkAnnotation(in: CGRect(x: 100.0, y: 100.0, width: 200.0, height: 200.0))
