...
- (void)updateDocumentWithMutatingFiles:(void (^)(NSMutableArray<NSURL *> *files))fileMutationBlock {
    ...
    [PSPDFKitGlobal.sharedInstance.cache removeCacheForDocument:self.rightDocument];
...
- (void)selectLeftSource:(id)sender {
    ...
    [self.leftController presentViewController:documentPicker options:@{ PSPDFPresentationOptionInNavigationController: @YES } animated:YES sender:sender completion:NULL];
}
