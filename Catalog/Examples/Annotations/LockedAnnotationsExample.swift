...
class LockedAnnotationsExample: Example {
    ...
    override func invoke(with delegate: ExampleRunnerDelegate) -> UIViewController {
        let document = AssetLoader.document(withName: AssetName.quickStart)

        document.annotationSaveMode = .disabled

        // Add some test annotations.
        let ink = InkAnnotation.psc_sampleInkAnnotation(in: CGRect(x: 100, y: 100, width: 200, height: 200))
        ink.color = UIColor.green
        let ink2 = InkAnnotation.psc_sampleInkAnnotation(in: CGRect(x: 300.0, y: 300.0, width: 200.0, height: 200.0))
        ink2.color = UIColor.black
        let ink3 = InkAnnotation.psc_sampleInkAnnotation(in: CGRect(x: 100.0, y: 400.0, width: 200.0, height: 200.0))
...
private class LockedAnnotationsPageView: PDFPageView {
    ...
    private func isColorAboutEqualToColorWithTolerance(left: UIColor?, right: UIColor?, tolerance: CGFloat) -> Bool {
        ...
        for i in 0..<componentCount {
            if CGFloat(abs(Float((leftComponents[i]) - (rightComponents[i])))) > tolerance {
                return false
            }
        }
