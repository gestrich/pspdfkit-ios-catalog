...
- (void)commonInitWithDocument:(PSPDFDocument *)document configuration:(PSPDFConfiguration *)configuration {
    ...
    UIBarButtonItem *closeButton = [[UIBarButtonItem alloc] initWithImage:[PSPDFKitGlobal imageNamed:@"x"] style:UIBarButtonItemStyleDone target:self action:@selector(closeButtonPressed:)];
...
- (void)askUserAboutSaveLocationIfNeeded {
    ...
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:nil message:@"Would you like to save annotations into the current file, or create a copy to save the annotation changes?" preferredStyle:UIAlertControllerStyleAlert];
...
- (void)replaceDocumentWithCopy {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [[self pageViewForPageAtIndex:self.pageIndex] showMenuIfSelectedWithOption:PSPDFContextMenuOptionAllowPopovers animated:NO];
    });
}
